...
var path = []
var walk_distance
var allier_speed = 300
var rot=0
const FIREBALL_SCENE = preload ("res://scene/fireball2.tscn")
var unit=[]
var pl=true
func _on_Timer_timeout():
	if unit.size() !=0:
		...
		fireball.position=$cannon/can.get_global_position()
		var d = Vector2 ($cannon/can.get_global_position() - get_global_position())
...
func _on_allier_input_event(_viewport, event, _shape_idx):
...
func _process(delta):
	if unit.size()!=0:
		for u in unit:
			n_rot= self.position.angle_to_point(u.position)-3.1
		$cannon.rotation=n_rot
	if moove:
